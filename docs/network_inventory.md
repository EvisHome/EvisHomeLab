...
{{ device.name }}	{{ device.type }}	{{ device.model }}	{{ device.ip if device.ip is defined else 'DHCP' }}	{{ device.location }}	{% if device.status == "Active" %} ● Active {% elif device.status == "Planned" %} ● Planned {% else %} ● Offline {% endif %}
